...
declare type ContainerId = string;
declare type DefaultReadinessCheck<T = void> = (arg0: T) => Promise<void>;
declare type ServiceName = string;
export interface DefaultReadinessChecks {
    postgres: DefaultReadinessCheck<{
        POSTGRES_DB: string;
...
export interface ReadinessCheck {
    ({ containerId, defaultReadinessChecks, dockerComposeFileService, dockerEventStream$, logger, }: {
...
export declare type DockerComposePortStringFormat = string;
export declare type DockerComposePortObjectFormat = {
    /** The publicly exposed port */
    published: number;
    /** The port inside the container */
    target: number;
};
export declare type DockerComposePortFormat = DockerComposePortStringFormat | DockerComposePortObjectFormat;
...
export declare type Commands = (string | ((containerId: string) => string))[];
...
declare type Jest = typeof import('jest');
declare type JestOpts = Partial<Parameters<Jest['runCLI']>[0]>;
...
export declare type TestRunModeType = 'docker-local-socket' | 'docker-injected-host-socket' | 'host';
